...
**`CH`/`DBU` are just the defaults.** A different restaurant sets `Store 1 Tab Prefix` and `Store 2 Tab Prefix` in the Settings tab to their own short codes and re-runs `initializeSheet` — it renames the tabs to match, in place, keeping all the data. (The restaurant names shown in the app come from `Location 1 Name` / `Location 2 Name`, which were already operator-editable.)
